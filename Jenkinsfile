pipeline {
    ...
    stages {
        stage('Clone GitHub Repo') {
            steps {
                script {
                    ...
                    checkout scmGit(branches: [[name: '*/main']], extensions: [], userRemoteConfigs: [[credentialsId: 'jenkins-token', url: 'https://github.com/kumar7ashutosh/medical-chatbot.git']])
                }
            }
        }
        ...
        stage('Build, Scan, and Push Docker Image to ECR') {
            steps {
                withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', credentialsId: 'aws-token1']]) {
                    script {
